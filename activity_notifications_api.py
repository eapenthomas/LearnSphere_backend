...
async def get_user_activities_since_login(user_id: str, last_login: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        grades_response = supabase.table("assignment_submissions").select("""
            ...
            courses!inner(title)
        """).eq("student_id", user_id).not_.is_("graded_at", "null").gte("graded_at", last_login).execute()
        
        for grade in grades_response.data or []:
            activities.append({
                'id': grade['id'],
                'type': 'assignment_graded',
                'title': f"Assignment Graded: {grade['assignments']['title']}",
                'message': f"Your assignment '{grade['assignments']['title']}' has been graded. Score: {grade['score']}",
                'priority': 'medium',
                ...
                'data': {
                    ...
                    'course_id': grade['assignments']['course_id'],
                    'score': grade['score'],
                    'course_title': grade['courses']['title']
                }
            })
        ...
        quiz_results_response = supabase.table("quiz_submissions").select("""
            ...
            courses!inner(title)
        """).eq("student_id", user_id).gte("submitted_at", last_login).execute()
        
        for result in quiz_results_response.data or []:
            activities.append({
                'id': result['id'],
                'type': 'quiz_graded',
                'title': f"Quiz Completed: {result['quizzes']['title']}",
                'message': f"You completed the quiz '{result['quizzes']['title']}' with a score of {result['score']}",
                'priority': 'medium',
                ...
                'data': {
                    ...
                    'course_id': result['quizzes']['course_id'],
                    'score': result['score'],
        ...
        for material in materials_response.data or []:
            activities.append({
                ...
                'message': f"New {material['material_type']} '{material['title']}' has been added to {material['courses']['title']}",
                'priority': 'medium',
        ...
        forum_response = supabase.table("forum_posts").select("""
            id, title, content, course_id, created_at, post_type,
            courses!inner(title),
        ...
        for post in forum_response.data or []:
            if post['post_type'] == 'question':
                activities.append({
                    ...
                    'message': f"A new question '{post['title']}' has been posted in {post['courses']['title']} forum",
                    'priority': 'low',
