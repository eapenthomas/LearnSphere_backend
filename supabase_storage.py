...
class SupabaseStorageManager:
    ...
    async def upload_file(self, file: UploadFile, course_id: str, bucket_name: str = "course-materials") -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate public URL
            public_url = self.supabase.storage.from_(bucket_name).get_public_url(file_key)
    ...
    async def upload_assignment_file(self, file: UploadFile, course_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Upload to Supabase Storage
            response = self.supabase.storage.from_("assignments").upload(
                path=file_key,
                file=file_content,
                file_options={
                    'content-type': content_type,
            ...
            # Check if upload was successful (newer Supabase client returns different response structure)
            ...
            # Generate public URL
            public_url = self.supabase.storage.from_("assignments").get_public_url(file_key)
    ...
    async def upload_course_thumbnail(self, file: UploadFile) -> Dict[str, Any]:
        ...
        try:
            ...
            # Upload to Supabase Storage
            response = self.supabase.storage.from_("course-thumbnails").upload(
                path=filename,
                file=file_content,
                file_options={
                    'content-type': content_type,
            ...
            # Generate public URL
            public_url = self.supabase.storage.from_("course-thumbnails").get_public_url(filename)
