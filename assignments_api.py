...
# Helper functions
async def get_current_user(user_id: str = Query(...)) -> Dict[str, Any]:
...
async def verify_student_role(user_id: str):
...
async def get_student_assignments(
    ...
    try:
        # Verify student role
        await verify_student_role(student_id)
        ...
        query = supabase.table("assignments_with_details").select("*").in_("course_id", enrolled_course_ids).eq("status", "active")
